...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        cX = -1
        cY = -1
        for loc in self.food.asList():
            if (loc != self.startState and loc not in self._visited):
                walls = self.walls
                assert not walls[x1][y1], 'point1 is a wall: ' + point1
                assert not walls[x2][y2], 'point2 is a wall: ' + str(point2)
                prob = PositionSearchProblem(gameState, start=self.startState, goal=loc, warn=False)
                self._visited[loc] = len(search.bfs(prob))
                self._visitedlist.append(loc)
            if (closest < self._visited[loc]):
                closest = self._visited[loc]
                cX, cY = loc
        return (cX, cY) == (x, y)
        # util.raiseNotDefined()
